function [res] = abs_react_OH(yvec,pars)
% ABS_REACT
...
pH      = pars(11); % const of velocity rating
...
j=1;
% first stage
res(j+0) = nL0*xCO2b0 + nCO2(1) - kOH*10^(-3)*cTOT*xCO2b(1)*cOH*1000*S*A - ...
           (nL(1)*xCO2b(1));                                               % CO2 bal in liq
...
for i=2:N-1
    res(j+0) = nL(i-1)*xCO2b(i-1) + nCO2(i) - kOH*10^(-3)*cTOT*xCO2b(i)*cOH*1000*S*A - ...
...
res(j+0) = nL(N-1)*xCO2b(N-1) + nCO2(N) - kOH*10^(-3)*cTOT*xCO2b(N)*cOH*1000*S*A - ...
           nL(N)*xCO2b(N);
